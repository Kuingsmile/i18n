...
const enhancedOptions: II18nConstructorOptions = { ...options, fallbackLanguages: ['fr', 'de'] as const }
const enhanced = createTypedI18n({ ...enhancedOptions, schema: { title: 'Title' } as const })
...
enhanced.selectPlural(1)
void [category, formattedNumber, formattedBigInt, formattedDate]

// @ts-expect-error Fallback chains contain only language strings.
new I18n({ ...options, fallbackLanguages: ['fr', 123] })
// @ts-expect-error A chain must be an array, not a string.
new I18n({ ...options, fallbackLanguages: 'fr' })
...
// @ts-expect-error The locale override is a string.
enhanced.formatNumber(1, undefined, 123)
